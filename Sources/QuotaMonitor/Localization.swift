...
enum L10n {
    ...
    private static let traditionalChineseTranslations: [String: String] = [
        ...
        "tiboPublicSourcesUnavailable": "無法連線公開 X 主頁與備用讀取服務。請檢查網路或 VPN，稍後再試，也可切換到官方 X API。",
        ...
        "tiboPublicProfileHelp": "只需 Tibo 的公開主頁連結，無需 X 開發者密鑰。應用會並行嘗試 X 公開嵌入頁面與備用唯讀服務；兩者都無法使用時可切換至官方 API。",
    ...
    private static let translations: [AppLanguage: [String: String]] = [
        .english: [
            ...
            "tiboPublicSourcesUnavailable": "The public X profile and backup reader could not be reached. Check your network or VPN, try again later, or switch to the official X API.",
            ...
            "tiboPublicProfileHelp": "Paste Tibo's public profile link; no X developer credential is needed. The app tries X's public embed and a read-only backup in parallel; use the official API if both are unavailable.",
        ...
        .simplifiedChinese: [
            ...
            "tiboPublicSourcesUnavailable": "公开 X 主页和备用读取服务均无法连接。请检查网络或 VPN，稍后重试，也可切换到官方 X API。",
            ...
            "tiboPublicProfileHelp": "只需粘贴 Tibo 的公开主页链接，无需 X 开发者凭证。应用会并行尝试 X 公开嵌入页面和备用只读服务；两者都无法使用时可切换到官方 API。",
        ...
        .japanese: [
            ...
            "tiboPublicSourcesUnavailable": "公開 X プロフィールと予備リーダーの両方に接続できません。ネットワークまたは VPN を確認し、後で再試行するか、公式 X API に切り替えてください。",
            ...
            "tiboPublicProfileHelp": "Tibo の公開プロフィール URL を貼るだけで、X 開発者認証情報は不要です。X の公開埋め込みと読み取り専用の予備サービスを並行して試し、両方が使えない場合は公式 API に切り替えられます。",
