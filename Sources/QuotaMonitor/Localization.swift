...
enum L10n {
    ...
    private static let traditionalChineseTranslations: [String: String] = [
        ...
        "primary": "額度",
        "tiboMissingXToken": "請先在設定中輸入 X API Bearer Token",
        ...
        "tiboSettingsHelp": "X 官方 API 需要獨立 Bearer Token。只有新推文會自動分析；手動檢查會強制重新分析最新一則。公開推文文字會傳送至所選模型服務，所有金鑰只儲存在本機鑰匙圈。"
    ]
    ...
    private static let translations: [AppLanguage: [String: String]] = [
        .english: [
            ...
            "primary": "Limit",
            "tiboMissingXToken": "Add an X API Bearer Token in Settings first",
            ...
            "tiboSettingsHelp": "The official X API requires a separate Bearer Token. Only new posts are analyzed automatically; a manual check reanalyzes the latest post. Public post text is sent to the selected AI provider, while every key stays in the local Keychain.",
        ...
        .simplifiedChinese: [
            ...
            "primary": "额度",
            "tiboMissingXToken": "请先在设置中输入 X API Bearer Token",
            ...
            "tiboSettingsHelp": "X 官方 API 需要单独的 Bearer Token。只有新推文会自动分析；手动检查会强制重新分析最新一条。公开推文文本会发送到所选模型服务，所有密钥只保存在本机钥匙串。"
        ],
        .japanese: [
            "appName": "AI 使用量",
            ...
            "primary": "上限",
            "tiboMissingXToken": "先に設定で X API Bearer Token を入力してください",
            ...
            "tiboSettingsHelp": "公式 X API には別の Bearer Token が必要です。新規投稿のみ自動分析し、手動確認では最新投稿を再分析します。公開投稿の本文は選択した AI プロバイダーへ送信され、すべてのキーはローカルのキーチェーンにのみ保存されます。"
        ]
    ]
}
