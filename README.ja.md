...
- Tibo と AI 通信に任意の HTTP/Mixed プロキシを使用可能。Clash で一般的な `127.0.0.1:7890` が初期値で、ポートの編集と接続テストに対応。
...
デフォルトの Tibo 監視は公開プロフィール URL `https://x.com/thsottiaux` だけで使用でき、X 開発者認証情報は不要です。X の公開プロフィール、公開埋め込みタイムライン、読み取り専用の予備サービスを並行して試します。すべての公開経路が使えない場合は、詳細設定で X API Bearer Token を使用できます。
...
- Tibo 分析が表示されない：設定で公開プロフィールと AI モデルをテストし、スパークルボタンを押します。公開経路で接続エラーが出る場合は、ネットワーク、VPN、またはプロキシを確認し、後で再試行するか、公式 X API に切り替えます。
- Clash の起動中に X へ接続できない：Tibo 設定でカスタムプロキシを有効にし、`127.0.0.1:7890` のままか Clash の現在の HTTP/Mixed ポートを入力し、「プロキシをテスト」の後に「公開プロフィールをテスト」を実行します。
