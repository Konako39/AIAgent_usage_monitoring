...
- Tibo（`@thsottiaux`）の新しいポストを監視し、現在の投稿、直前の 1 投稿、公開された人気の返信を最大 3 件まで OpenAI、Claude、Gemini、DeepSeek のいずれかに送り、GPT / Codex 使用量リセットの明示的または暗示的なシグナルを判定。
...
設定で入力した認証情報は macOS キーチェーンに保存されます。設定を開く操作や公開プロフィールのテストで、保存済みのキーを読み取ることはありません。明示的にモデルをテストするとき、または新しいポストの AI 分析が必要なときだけキーチェーンにアクセスします。ポストに変化がないバックグラウン確認もアクセスしません。使用量履歴、最近 3 件のタスク、最新の Tibo 分析は Mac 内にのみ保存されます。新しい Tibo のポスト、直前の投稿を最大 1 件、公開された高エンゲージメントの返信を最大 3 件、選択した AI プロバイダーに送信します。
